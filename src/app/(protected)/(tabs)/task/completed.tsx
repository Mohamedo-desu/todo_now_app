...
const CompletedTasks = () => {
  const {
    ...
    api.tasks.fetchCompletedTasks as PaginatedQueryReference,
    {},
    { initialNumItems: 10 }
  );
  ...
  return (
    <AnimatedLegendList
      ...
      ListEmptyComponent={
        isLoading ? <Loader size="small" /> : <Empty text="No completed tasks found" />
      }
      ListFooterComponent={
        status === 'LoadingMore' ? (
          <Loader size="small" />
        ) : status === 'Exhausted' && tasks.length !== 0 ? (
          <Empty text="You don't have any more completed tasks" />
        ) : null
      }
    />
  );
